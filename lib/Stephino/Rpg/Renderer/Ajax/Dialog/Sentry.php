...
class Stephino_Rpg_Renderer_Ajax_Dialog_Sentry extends Stephino_Rpg_Renderer_Ajax_Dialog {
    ...
    public static function ajaxInfo($data) {
        ...
        // Reset sentry active flag
        if (null === $sentryConvoy) {
            $sentryOwnerData[Stephino_Rpg_Db_Table_Users::COL_USER_SENTRY_ACTIVE] = 0;
            Stephino_Rpg_Db::get()->tableUsers()->updateById(
                array(
                    Stephino_Rpg_Db_Table_Users::COL_USER_SENTRY_ACTIVE => 0
                ), 
                $sentryOwnerData[Stephino_Rpg_Db_Table_Users::COL_ID]
            );
        }
